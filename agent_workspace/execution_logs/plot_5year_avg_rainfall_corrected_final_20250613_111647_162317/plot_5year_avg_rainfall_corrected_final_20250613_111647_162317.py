...
for product in rainfall_products:
    ...
    try:
        ...
        # 确保时间维度是最后一个维度，如果是 (时间, 纬度, 经度) 需要转置
        # 这里假设时间维度是最后一个 (即轴2，索引为2)
        # 如果是 (时间, 纬度, 经度) 则需要转置为 (纬度, 经度, 时间)
        if rainfall_data.shape[0] == 1827 and len(rainfall_data.shape) == 3: # 假设时间维度是第一个维度
        ...
        # 确保时间维度是1827
        if len(rainfall_data.shape) < 3 or rainfall_data.shape[2] != 1827:
            print(f"Warning: Unexpected time dimension for {product}: {rainfall_data.shape}. Expected (lat, lon, 1827). Skipping.")
            continue
