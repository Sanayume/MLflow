...
# Ensure GSMAP and IMERG are loaded for filling
if all_product_data.get('GSMAP') is None or all_product_data.get('IMERG') is None:
    ...
    # Fallback to 0 fill if source products are missing or not loaded
    fallback_fill_method = 'zero'
else:
...
for product_name_to_fill in ['CHIRPS', 'sm2rain']:
    ...
    if fallback_fill_method == 'zero':
        ...
        # np.array([gsmap_values, imerg_values]) creates a 2xN array. nanmean along axis=0 computes mean for each column
