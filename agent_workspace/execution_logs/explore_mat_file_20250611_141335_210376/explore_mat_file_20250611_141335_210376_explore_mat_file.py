...
try:
    mat_file_path = '/sandbox/datasets/rainfalldata/CHIRPSdata/chirps_2016.mat'
    ...
    # 假设 'chirps' 或类似名称是实际数据
    if 'chirps' in data:
        print(f"变量 'chirps' 的形状: {data['chirps'].shape}")
        print(f"变量 'chirps' 的数据类型: {data['chirps'].dtype}")
