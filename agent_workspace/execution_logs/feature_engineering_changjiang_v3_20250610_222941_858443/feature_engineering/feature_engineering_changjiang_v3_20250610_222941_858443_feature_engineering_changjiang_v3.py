...
# --- Custom Functions ---
def nan_range(arr, axis=None):
    """
    Calculates the range (max - min) of an array, ignoring NaNs.
    Behaves like np.ptp but with NaN handling.
    """
    if arr.size == 0:
        return np.nan # Handle empty array

    # If axis is None, flatten the array for calculation
    if axis is None:
        flat_arr = arr.flatten()
        if np.all(np.isnan(flat_arr)):
            return np.nan
        return np.nanmax(flat_arr) - np.nanmin(flat_arr)
    else:
        # Calculate max and min along the specified axis
        max_vals = np.nanmax(arr, axis=axis)
        min_vals = np.nanmin(arr, axis=axis)

        # Handle cases where all values along an axis slice are NaN
        # If max_vals or min_vals is NaN, their difference will be NaN, which is correct.
...
# Map product names to their respective directory names
PRODUCT_INFO = {
    'CMORPH': 'CMORPHdata',
    'CHIRPS': 'CHIRPSdata',
    'GSMAP': 'GSMAPdata',
    'IMERG': 'IMERGdata',
    'PERSIANN': 'PERSIANNdata',
    'SM2RAIN': 'sm2raindata', # Special case for directory name
    'CHM': 'CHMdata'
}
PRODUCTS = list(PRODUCT_INFO.keys()) # List of product names
...
for product_name, dir_name in PRODUCT_INFO.items():
    print(f"Processing product: {product_name}")
    product_file = os.path.join(RAINFALL_DATA_DIR, dir_name, f"{product_name}_2016_2020.mat")
    try:
        product_full_data_raw = scipy.io.loadmat(product_file)['data'].astype(np.float32)
        # Transpose data from (lat, lon, time) to (time, lat, lon)
        product_full_data = product_full_data_raw.transpose(2, 0, 1)
        del product_full_data_raw
        ...
        if product_full_data.shape[0] != N_days or product_full_data.shape[1] != N_lat or product_full_data.shape[2] != N_lon:
            print(f"Warning: {product_name} data shape {product_full_data.shape} after transpose does not match expected ({N_days}, {N_lat}, {N_lon}). Skipping.")
    ...
    for window in [3, 7, 15]:
        for stat_func, stat_name in [(np.nanmean, 'mean'), (np.nanstd, 'std'),
                                     ...
                                     (nan_range, 'range')]: # Use the custom nan_range function
